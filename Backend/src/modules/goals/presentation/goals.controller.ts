...
import { CreateGoalDto, CreateMilestoneDto, GoalQueryDto, UpdateGoalDto, UpdateMilestoneDto } from './dto/goals.dto';
@ApiTags('goals')
@ApiBearerAuth('access-token')
@Controller('goals')
export class GoalsController {
  constructor(private readonly goals: GoalsService) {}
  @Get() list(@CurrentUser() u: AuthenticatedUser, @Query() q: GoalQueryDto) {
    return this.goals.list(u.id, q);
  }
  @Post() create(@CurrentUser() u: AuthenticatedUser, @Body() d: CreateGoalDto) {
  ...
  @Patch(':id') update(@CurrentUser() u: AuthenticatedUser, @Param('id') id: string, @Body() d: UpdateGoalDto) {
